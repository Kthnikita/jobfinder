...
async function getsugg(){
  const req=await fetch("http://localhost:3000/api/search/suggestion?q="+job);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center w-full md:w-auto gap-2 md:gap-4">
          <div className="flex-grow relative">
            ...
            {sugg.length==0?<div></div>: <div className="absolute top-12 w-[400px] h-[200px] bg-white shadow-md shadow-blue-700 flex flex-col rounded-md p-2">
                {sugg.map((val,ind)=>{
                  return(
                    <div key={val.id} className="">
                      <p className="flex gap-1 text-gray-600 hover:text-blue-500 truncate"><Search/>{val.title}</p>
          ...
          >
            ...
            {dialog && (
              <div className="absolute right-0 mt-2 w-64 bg-white text-black p-4 rounded-lg shadow-lg z-50">
                ...
                <hr className="my-2" />
                <Link href="/" className="flex text-sm mb-2 hover:text-blue-700 text-gray-900 gap-2">
                 <House className="hover:text-blue-700 text-gray-900"/>
                </Link>
                ...
                <button className="block w-full text-left text-red-600 hover:underline text-sm" onClick={haandellogout}>
                  Logout 
                </button>
              </div>
            )}
          </div>:<Link href="/login" className="h-10 w-18 bg-blue-400 text-white text-center p-2">LOGIN</Link>}
        </div>
      </div>
    </header>
  );
